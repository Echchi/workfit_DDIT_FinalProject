...
input[name="customRadio1"]:checked + label {
/* 	font-size:17px;  */
	color:white; 
	...
	<div class="col-12">
			<div class="card mr-2 workfitCardBg commonCard">
				...
				<%-- 		<input type="hidden" name="postCode" value="${postVO.postCode}" /> --%>
					...
					<div class ="row">
						<div class="col-md-4">
							<c:if test="${bstatus eq 'notice' }"> 
								<h5 id="headTitle" class='apple4 page-title text-dark ml-4' style='font-size: 31px; letter-spacing: 2px;'>공지사항 등록</h5>
							</c:if>
							<c:if test="${bstatus eq 'free' }">
								<h5 id="headTitle" class='apple4 page-title text-dark ml-4' style='font-size: 31px; letter-spacing: 2px;'>자유게시판 등록</h5>
							</c:if>
						</div>
					</div>		
						
					<div class="row pt-2 pl-3">
						<div class="row col-12 md-2 ml-2 pl-2">
							<label style="font-size:19px;" for="colFormLabel" class="apple3 text-dark col-form-label" >제목</label>
							<div class="col-md-10">
								<input type="text" class="apple3 text-dark mt-1" style="font-size: 19px;background-color:inherit; position : relative; display : inline-block; width : 960px;border-left-style: none; border-top-style: none; border-right-style: none; border-bottom : 0.5px solid gray;border-radius:none;" name="postTitle" id="simpleinput" placeholder="제목을 작성해주세요" value="${postVO.postTitle}">
							</div>
						</div>
		<!-- 							</div> -->
		<!-- 								<div style="width : 1000px;" class="col-sm-10"></div> -->
		<!-- 							<div class="row"> -->
						<c:if test="${bstatus eq 'notice' }"> 
							<div class="mt-1 ml-4">
							    <div class="custom-control custom-radio custom-control-inline">
							        <input type="radio" id="customRadio4" checked="checked" name="customRadio1" class="custom-control-input" value='n'>
							        <label class="custom-control-label apple3 text-dark" for="customRadio4" >팝업 설정 X &nbsp;&nbsp;</label>
							    </div>
							    <div class="custom-control custom-radio custom-control-inline">
							        <input type="radio" id="customRadio3" name="customRadio1" class="custom-control-input" value='y'>
							        <label class="custom-control-label apple3 text-dark" for="customRadio3" >팝업 설정 O &nbsp;&nbsp;</label>
							    </div>
							</div>
						</c:if>
					</div>
				<div class="pl-3 pr-4 mb-2 mt-2">
									<label style="width : 100px; font-size:19px;" for="colFormLabel" class="col-form-label apple3 text-dark pl-2">내용</label>
									<button type="button" id="formBtn" class="btn btn-primary workfitBtn apple3 " style="font-size:17px;float:right;" value="${name }">${name }</button>
				</div>
				<div class="ml-4">
					<textarea class="pl-3 ml-4" id="summernote" rows="30" cols="30" name="postContent">${postVO.postContent }</textarea>
				</div>
<%-- 					<textarea id="ckeditor" rows="30" cols="30" name="postContent">${postVO.postContent }</textarea> --%>
					<br/>
				...
				<!-- 								<a href="#" class="mailbox-attachment-name"> -->
				 								 ...
				 								 <i	class="fas fa-paperclip"></i> ${boardAttach.boardAttachName }
...
<!-- 					<span> 첨부 파일 </span> -->
					<div class="col-2 md-1 pl-3">
						<label style="width : 100px; font-size:19px;" for="colFormLabel" class="col-form-label apple3 text-dark pl-2">첨부 파일</label>
					</div>
					<div class="form-group col-12 md-4" style="margin: -20px;height: 200px;">
							<!-- DropZone SSB -->
							<div class="dz-message needsclick" data-upload-preview-template="#uploadPreviewTemplate" style="background-color: white;height:160px;margin -10px;border-radius: 0.8rem;width: 1500px;margin-left: 50px;">
								<i class="h1 text-muted dripicons-cloud-upload" style="position: relative;top:7%;font-size: 50px;"></i>
								<h3 style="margin-bottom: -5px; ">첨부할 파일을 올려주세요</h3>
								<span class="text-muted font-13">(파일은 <strong>최대 5개</strong>까지 올리실 수 있습니다)</span>
							</div>
							<!-- Preview -->
							<div class="dropzone-previews mt-3" id="file-previews"></div>
<!-- 							<div class="dropzone-previews mt-3" id="dropzone-preview"></div> -->
...
var popUpStatus = $("#test");
$('input[type=radio][name=customRadio1]:checked').change(function(e) {console.log(e)})

$(function() {
    document.querySelector('#test').value='n';
//     CKEDITOR.replace('postContent');

  $('#headTitle').click(function(){
	  console.log("?????");
	  $('#simpleinput').val("2023년 야유회 일정 공지");
	  $('#summernote').summernote('pasteHTML',"2023년 2월 21일 ~ 2월 22일까지 사내 야유회 일정 공지합니다.<br/>	  첨부 파일 확인해주십시오.");
  })
  $('input[type=radio][name=customRadio1]:checked').change(function(e) {
...
// 		}
	...
	var popUpSubmit =$('input[type=radio][name=customRadio1]:checked');
// 											popUpStatus.val(popUpSubmit.val);
...
// 	alert(boardType);
		var dropzone = new Dropzone("form.dropzone",
					...
					paramName : 'file', // 서버에서 사용할 formdata 이름 설정 (default는 file... parameter로 날아갑니당.)
...
// 					previewsContainer: '#dropzone-preview',
					...
					init : function() {
						...
						document.querySelector("#formBtn").addEventListener("click",function(e, file) {
											let checkPopup  =$('input[type=radio][name=customRadio1]:checked').val();
							 				test.val(checkPopup);
							 				if(checkPopup =="y"){
							 					toggleMainPopup(checkPopup);
							 				}
											e.preventDefault();			// Form 전송 버블링 방지.
											e.stopPropagation();
											if (myDropzone.getQueuedFiles().length > 0) {
// 												alert("파일 이 드러왔따...!"	+ myDropzone.getQueuedFiles().length);
												myDropzone.processQueue();
// 												let postContent = CKEDITOR.instances.postContent.getData();
// 												console.log('postContent 값',postContent);
												$("#formBtn").submit();
												return;
											}
// 											alert('그냥 일반 입력이닷!');
						...
						this.on("queuecomplete", function(file) {
							...
							console.log('큐 완료...',boardType);
							// 첨부 파일이 있을 경우 리스트로 보내준다.
