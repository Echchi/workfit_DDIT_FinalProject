...
<div class="container" id="mainContainer" style="margin-left: 184px; max-width: 1578px;">
	<%
		int info = (int)request.getAttribute("serviceInfo");
		if(info > 2){
	%>
		<button class="pt-2 pb-2  apple3" onclick="openWidget(this)" style="font-size:19px;position:absolute;top:25px;left:75%;width:230px;color:white;border:none;border-radius:2rem;background-color: rgba(255,255,255,0.5)">대시보드 커스텀</button>
	<%
		}
	%>
	<div class="row mt-5 mainRow" data-plugin=""></div>
</div>
		...
		swal({
			...
			.then((willDelete) => {
			 ...
			 // ----------------------- 퇴근 처리 위한 로직 작성 시작 1.27 ---------------------------------
				...
				$("#commuteEndClock").css("font-size",23+"px;");
...
//	 			imgTag.src = res.url; 
	...
	if(sessionCommuteFlag == "end"){
		...
		$("#commuteStartClock").css("font-size",70+"px;");
...
function getMailSuc(list){
	...
	var count = 0;
			...
			for(var item of list){
					console.log("메일 제목",item.emailTitle)
					count++;
					var str = '<div class="text-dark apple6 justify-content-between align-items-center row text-dark pt-2 pl-3 pr-3 mr-1 ml-1 mb-2" style="border-radius: 20px; background-color: #f8f9fc; font-size: 19px;">'
					str +="<p class='mr-3'>"+item.memName+"</p>"
					var mailTitle = item.emailTitle == null ? " - " : item.emailTitle 
					str +="<p style='overflow: hidden;text-overflow: ellipsis;white-space: nowrap;word-break:break-all;width: 200px;'>"+mailTitle+"</p>"
					
					var date = dayjs(item.emailDate);
					str +="<p class='ml-3' style='font-size:17px;'>"+date.format("YYYY.MM.DD HH:mm")+"</p>"
					str +="</div>"
...
// 				console.log("list.length",list.length)
			...
			for(var item of list){
					count++;
					str += '<div class="text-dark apple6 justify-content-between align-items-center row text-dark pt-2 pl-3 pr-3 mr-1 ml-1 mb-2" style="border-radius: 20px; background-color: #f8f9fc; font-size: 19px;">'
...
// 	console.log("toDoFuncSuc 성공후 그리는 메소드 실행 ")
	...
	for(let item of list){
		...
		str +=`<div class="ml-3"><h4 style="font-size:21px;" class="apple5 text-dark text-left ml-3" data-memoid= "\${item.todayCode}" id="\${item.todayCode}"><input type="checkbox" \${checkVal} style="accent-color:#6c757d" class="mr-3 chkToDo" onchange="checkTodo(this)"> \${item.todayContent}`
		str += `<button onclick="deleteToDo(this)" class="ml-3" style="position:absolute; left:85%; background-color: inherit;border: none;">x</button></h4></div>`;
...
//         	h4.setAttribute("style","font-size:24px;")
//         	//console.log(h4);
...
function checkTodo(item){
	...
	var checkItems = document.querySelector(".mainRow h4[data-memoId='"+todayCode+"']") 	
		...
		console.log("체크된 것과 같은 아이디의 모든 체크박스", target.checked)
		
		var check = target.checked? 1: 0;
			var map = {
						"todayCode" : todayCode,
			 			"check" : check
						}	
...
// 	console.log("위젯 컨텐트 div", newsContent);
	...
	var ContentHTML = "";
	ContentHTML += `<h4 class="apple5 text-dark mb-2" style="font-size: 22px">정치 분야 상위 뉴스</h4>`
	for(let item of json){
		...
		ContentHTML += 		`<div class="col-9">`
		ContentHTML += 		`	<h4 class="apple4 text-dark">\${item.newsTitle}</h4>`
...
function getDeptCalSuc(list){
	...
	deptTodayDate.innerHTML = "<div class='apple5 text-dark'style='font-size:21px;'>"+todayDateWeek+"요일</div>";
	deptTodayDate.innerHTML += "<div class='text-center apple8 text-dark'style='font-size:45px;'>"+todayDate+"</div>";
	
	var deptCalTitle = document.querySelector("#deptCalTitle");
	deptCalTitle.innerHTML = '<i class="dripicons-calendar text-dark mr-2"></i>'+list[0].deptName + " 일정";
	var count = 0;
			if(!list[0].color){
				console.log("list.length",list.length)
				var str = "<div class='d-flex justify-content-center align-items-center'>"
				str += "<p class='text-dark apple6 text-center ml-3' style='font-size:23px;'>일정이 없습니다 </p>"
				str += "</div>"
			...
			for(var item of list){
					...
					str +='<span><img src="/resources/assets/images/'+color+'.png" height="20px" width="20px" class="ml-2 mr-3"></span>'
					str +="<div class='text-dark apple6 mr-3 text-left' style='white-space: nowrap;word-break:break-all;width: 200px; font-size: 25px;'>"+item.title+"</div>"
// 					str += "</div>"
					...
					if(item.allDay){
						...
						str += "<div class='apple4 text-right' style='font-size:19px;white-space: nowrap;'>"+startTime.format("HH:mm")+" - "+endTime.format("HH:mm") +"</div>"
					}
					str += "</div>"
			 	...
			 	if(count < 2){ 
					...
					str += "<div class='apple4 mr-3' style='width:50px;font-size:19px;> 그 외 일정"+(list.length - 2) +"</div>"
					deptToday.innerHTML += str;
					break;
				}
...
function getMyCalSuc(list){
	...
	myTodayDate.innerHTML = "<div class='apple5 text-dark'style='font-size:21px;'>"+todayDateWeek+"요일</div>";
	myTodayDate.innerHTML += "<div class='text-center apple8 text-dark'style='font-size:45px;'>"+todayDate+"</div>";
	...
	var count = 0;
			...
			for(var item of list){
					...
					var color = item.color.substring(1)
					var str = "<div class='d-flex align-items-center'>"
					str +='<span><img src="/resources/assets/images/'+color+'.png" height="20px" width="20px" class="ml-2 mr-3 mb-1"></span>'
					str +="<div class='text-dark apple6 mr-3 text-left' style='overflow: hidden;text-overflow: ellipsis;white-space: nowrap;word-break:break-all;width: 200px; font-size: 23px;'>"+item.title+"</div>"
// 					str += "</div>"
					
					if(item.allDay){
						str += "<div class='apple4 text-right' style='font-size: 19px'>하루종일</div>"
					}else{
						var startTime = dayjs(item.start); 
						var endTime = dayjs(item.end); 
						str += "<div class='apple4 text-right' style='font-size:19px;white-space: nowrap;'>"+startTime.format("HH:mm")+" - "+endTime.format("HH:mm") +"</div>"
...
var drawMemStatus = (data) => {
	...
	*/
	...
	for(let item of data){
		...
		str += '		<div class="img">';
		if(item.memImg){
			str += '			<img alt="ㅠ" class="rounded-circle"src="'+item.memImg+'" width="45px" hegiht: 45px; style="box-shadow: 1px 1px 10px #565656;">';
		}else{
			str += '			<div class="rounded-circle text-center" style="margin: auto; font-size: 22px; line-height: 45px; width : 45px; height: 45px; box-shadow: 1px 1px 10px #565656; background-color : #ededed; ">'+ item.memName.substring(0,1) +'</div>';
		}
